    ...
    <div className="relative rounded-2xl overflow-hidden bg-dark-text/90 shadow-soft hover:-translate-y-1 hover:shadow-[0_12px_40px_rgba(0,0,0,0.45)] transition-all duration-300">
      {error ? (
        <div className="w-full h-52 bg-cream flex items-center justify-center">
          <span className="font-inter text-sm text-dark-text/40">Video coming soon</span>
        </div>
      ) : (
        <div className="aspect-video" style={{ background: 'radial-gradient(ellipse at center, #2d2d2d 0%, #0a0a0a 100%)' }}>
          <video
            ref={ref}
            src={src}
      ...
      {!playing && !error && (
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/65 to-transparent z-10">
            <p className="font-inter text-[10px] text-gold tracking-widest uppercase mb-1">{category}</p>
            <p className="font-playfair text-white text-lg leading-snug">{title}</p>
          </div>
        </div>
      )}
    </div>
...
export default function Gallery() {
  useSEO({
    ...
    description: "See our haircut transformations, beard styling work, and premium salon interior. Bliss Haven Edge — precision craftsmanship meets luxury in Wah Cantt.",
    path: '/gallery',
  })
  ...
  const filtered = active === 'all'
    ...
    : active === 'haircuts-beard'
      ? GALLERY_IMAGES.filter((img) => img.category === 'haircuts' || img.category === 'beard')
  ...
  return (
    <div className="overflow-x-hidden">
      ...
      <section className="bg-white px-4 pt-14 pb-10 text-center">
        ...
        <motion.p {...scrollFade(0.2)} className="font-inter text-dark-text/55 text-sm max-w-md mx-auto leading-relaxed">
          Experience the intersection of precision craftsmanship and architectural serenity.
          Every cut, every detail, every moment is designed for the modern gentleman.
        </motion.p>
      </section>
      ...
      <section className="bg-[#1a1a1a] px-4 py-16">
        <div className="max-w-2xl mx-auto">
          <motion.p {...scrollFade()} className="font-inter text-[11px] tracking-[0.22em] uppercase text-white/35 text-center mb-3">
            In Motion
          </motion.p>
          <motion.h2 {...scrollFade(0.1)} className="font-playfair text-3xl md:text-4xl text-white text-center mb-10">
            Transformations
          </motion.h2>
      ...
      <section className="bg-[#1a1a1a] py-20 px-4 text-center">
        <motion.h2 {...scrollFade()} className="font-playfair text-4xl md:text-5xl text-white mb-4 leading-tight">
