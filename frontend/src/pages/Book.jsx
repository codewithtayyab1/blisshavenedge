...
function ServiceDropdown({ selected, onToggle, error }) {
  ...
  return (
    <div ref={wrapRef} className="relative">
      ...
      >
        <span
          className={`font-inter text-sm truncate ${triggerText ? "text-dark-text" : "text-dark-text/35"}`}
        >
            ...
            {SERVICE_CATEGORIES.map((cat) => (
              <div key={cat.id}>
                <div className="sticky top-0 z-[1] px-4 py-1.5 bg-cream/95 border-b border-dark-text/8">
                  <span className="font-inter text-[9px] tracking-[0.2em] uppercase text-dark-text/45 font-semibold">
                    {cat.name}
                  </span>
                </div>
                {cat.services.map((svc) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="font-inter text-xs text-dark-text/45 shrink-0 tabular-nums">
                        {fmtPrice(svc.price)}
                      </span>
                    </label>
                  );
            ...
            {DEALS.map((deal) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="font-inter text-sm text-dark-text flex-1">
                    {deal.name}
                    <span className="text-dark-text/40 text-xs ml-1.5">
                      ({deal.includes.join(" + ")})
                    </span>
                  </span>
                  <span className="font-inter text-xs text-dark-text/45 shrink-0 tabular-nums">
            ...
            {SPECIAL_OFFERS.map((offer) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="font-inter text-[11px] text-gold font-medium shrink-0">
                    FREE offer
                  </span>
                </label>
              );
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-5 py-5 text-center">
        ...
        {preview && (
          <img
            ...
            className="mt-3 mx-auto max-h-28 rounded-lg object-cover shadow-sm"
          />
        )}
      </div>
    );
      ...
      <div className="bg-red-50 border border-red-200 rounded-xl px-5 py-5">
        {preview && (
          <img
            src={preview}
            alt="Preview"
            className="mx-auto max-h-24 rounded-lg mb-3 object-cover"
          />
        )}
        <p className="font-inter text-sm text-red-600 mb-3">{errMsg}</p>
        <button
          onClick={reset}
          className="w-full bg-red-500 hover:bg-red-600 text-white font-inter text-sm py-2.5 rounded-xl transition-colors"
        >
          Try Again
        </button>
      </div>
      ...
      <div className="bg-cream rounded-xl px-5 py-5">
        {preview && (
          <img
            src={preview}
            alt="Preview"
            className="mx-auto max-h-28 rounded-lg mb-4 object-cover shadow-sm"
          />
        )}
        ...
        <p className="font-inter text-xs text-dark-text/55 text-center">
          Uploading… {progress}%
        </p>
      </div>
    );
  ...
  return (
    <div>
      ...
      >
        ...
        <span className="font-inter text-sm font-medium text-dark-text">
          Upload Payment Screenshot
        </span>
        <span className="font-inter text-[11px] text-dark-text/45">
          Tap to choose a photo · max 5 MB
        </span>
      </button>
    </div>
  );
...
function downloadSlip({ referenceNumber, booking }) {
  const servicesList = (
    ...
    .map((s) => `<li style="padding:2px 0">${s}</li>`)
    .join("");
  ...
  return (
    <div>
      ...
      <section className="bg-[#1a1a1a] px-4 pt-16 pb-12 text-center">
        ...
        >
          {uploaded
            ? "Booking Confirmed — Payment Proof Received!"
            : "Booking Confirmed!"}
        </motion.h1>
        <motion.p
          initial={{ opacity: 0 }}
        ...
        >
          <p className="font-inter text-[10px] tracking-[0.22em] uppercase text-gold/65 mb-1">
            Reference Number
          </p>
          <p className="font-playfair text-5xl text-gold tracking-widest">
            {referenceNumber}
          </p>
          <p className="font-inter text-[10px] text-white/30 mt-1">
        ...
        <div className="max-w-md mx-auto space-y-4">
          ...
          <div className="bg-white rounded-2xl shadow-card overflow-hidden">
            <div className="px-5 py-3 border-b border-dark-text/6 bg-cream/50">
              <p className="font-inter text-[10px] tracking-widest uppercase text-dark-text/40">
                Appointment Details
              </p>
            </div>
            <div className="divide-y divide-dark-text/6">
              {[
                ...
                >
                  <span className="font-inter text-[10px] tracking-widest uppercase text-dark-text/40">
                    {k}
                  </span>
                  <span className="font-inter text-sm text-dark-text font-medium">
                    {v}
                  </span>
                </div>
              ))}
              ...
              {booking.services?.length > 1 ? (
                <div className="px-5 py-3">
                  <span className="font-inter text-[10px] tracking-widest uppercase text-dark-text/40 block mb-2">
                    Services
                  </span>
                  <ul className="space-y-1">
                    {booking.services.map((s) => (
                      <li
                        key={s}
                        className="font-inter text-sm text-dark-text font-medium text-right"
                      >
                        {s}
                      </li>
                    ))}
              ...
              {booking.price && (
                <div className="flex justify-between items-center px-5 py-3 bg-gold/5 rounded-b-2xl">
                  <span className="font-inter text-[10px] tracking-widest uppercase text-gold">
                    Total Due
                  </span>
          ...
          <div className="bg-white rounded-2xl shadow-card overflow-hidden">
            <div className="px-5 py-3 border-b border-gold/15 bg-gold/5">
              <p className="font-inter text-[10px] tracking-widest uppercase text-gold">
                Payment Instructions
              </p>
            </div>
            <div className="px-5 py-5">
              <p className="font-inter text-sm text-dark-text/65 leading-relaxed mb-2">
                To confirm your booking faster, you can send an advance payment
                via Easypaisa or JazzCash to{" "}
                <span className="font-medium text-dark-text">
                  0303-9228634 (Ali Hassan)
                </span>{" "}
              ...
              <p className="font-inter text-sm text-dark-text/50 leading-relaxed mb-4">
                ...
                — you can also simply arrive at your appointment time and pay at
                the salon.
                {booking.price && (
              ...
              <div className="bg-cream rounded-xl px-4 py-3.5 space-y-2 mb-5">
                <div className="flex justify-between items-center">
                  <span className="font-inter text-xs text-dark-text/45">
                    Easypaisa / JazzCash
                  </span>
                  <span className="font-inter text-sm font-semibold text-dark-text">
                    {payment.easypaisa.number}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="font-inter text-xs text-dark-text/45">
                  ...
                  <span className="font-inter text-sm text-dark-text">
                    {payment.easypaisa.name}
                  </span>
                </div>
              </div>
              <ScreenshotUpload
                bookingId={booking._id || booking.referenceNumber}
          ...
          <Link
            to="/"
            className="block text-center font-inter text-sm text-dark-text/40 hover:text-gold transition-colors py-2"
          >
            ← Back to Home
          </Link>
        </div>
...
export default function Book() {
  useSEO({
    ...
    description:
      "Book your grooming appointment online at Bliss Haven Edge, Wah Cantt. Choose your service, pick a time, and confirm with a quick advance payment via Easypaisa or JazzCash.",
    path: "/book",
  });
  ...
  useEffect(() => {
    if (state?.service) {
      setForm((f) => ({
        ...f,
        services: f.services.includes(state.service)
          ? f.services
          : [...f.services, state.service],
      }));
    }
  }, [state?.service]);
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            >
              ...
              >
                ...
                <div>
                  ...
                  >
                    ...
                    {form.date === TODAY &&
                      getAvailableSlots(form.date).length === 0 && (
              ...
              {apiError && (
                ...
                >
                  <p className="font-inter text-sm text-red-600 mb-2">
                    {apiError}
                  </p>
                  <a
